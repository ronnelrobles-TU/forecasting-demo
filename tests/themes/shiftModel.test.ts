...
describe('scheduledCountAt', () => {
  ...
  it('reads the right interval', () => {
    ...
    expect(scheduledCountAt(ramp, 720)).toBe(ramp[24].agents) // noon
    expect(scheduledCountAt(ramp, 1380)).toBe(ramp[46].agents) // 11pm
  })

  it('clamps to last interval when overshooting', () => {
    expect(scheduledCountAt(ramp, 9999)).toBe(ramp[47].agents)
  })
})
...
describe('smoothScheduledAt', () => {
  ...
  it('interpolates between neighbour intervals', () => {
    // At minute 15 (halfway through interval 0), should be halfway from
...
describe('isAgentActive', () => {
  ...
  it('peak at noon: most agents active', () => {
    ...
    expect(active).toBeGreaterThanOrEqual(90)
  })
  ...
  it('low at 11pm: skeleton crew', () => {
    ...
    // ramp[46].agents ≈ 13. Allow some jitter from stagger.
...
describe('smoothOfficeAllocation (Round 7.1)', () => {
  ...
  it('productive equals Erlang count when shrinkPct is 0', () => {
    ...
    // start of interval 24 (minute 720) it reads halfway between ramp[23]=96
    // and ramp[24]=100 → ~96. Use a boundary that lands cleanly on 100.
    const a = smoothOfficeAllocation(ramp, 720 + 29.99, 0)
  ...
  it('inOffice = productive / (1 - shrink/100) at peak', () => {
    const a = smoothOfficeAllocation(ramp, 720 + 29.99, 32)
    expect(a.productive).toBe(100)
